...
1. **Identify the site type & capability gating**:
   ...
   - **Capability Gating Matrix**:
     ...
     - *Push-Capable Publishing Stack*: A CMS, CDN/edge integration, publish webhook, worker, or deployment pipeline can push changed URLs → triggers `indexnow-instant-indexing-audit`. (If the site has no mechanism that could push URL changes, skip it; IndexNow findings are non-scoring per the rubric.)
     - *Operator-Supplied Answer Transcripts*: The operator supplies pasted or exported answer transcripts for the target brand, query corpus, and tracked competitor cohort → triggers `ai-share-of-voice-audit`. (If transcripts are absent, skip it — do not query answer engines or third-party observation APIs on the operator's behalf. Its sample statistics are non-scoring per the rubric.)
...
| **Instant URL Discovery** | `indexnow-instant-indexing-audit` | Audits IndexNow key hosting and path scoping, configured submission payloads, and CMS/edge push triggers without submitting URLs unless the operator explicitly authorizes it. |
| **AI Share of Voice** | `ai-share-of-voice-audit` | Computes cohort-scoped brand mentions, citation share of voice, and competitor displacement from operator-supplied answer transcripts; never queries answer engines itself. |
