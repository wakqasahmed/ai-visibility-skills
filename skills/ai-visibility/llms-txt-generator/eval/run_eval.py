#!/usr/bin/env python3
"""Deterministic contract layer for the llms-txt-generator skill.
...
plausible drafting/review request plus the compliant output a correctly-
behaving agent following SKILL.md and references/checks.md would produce)
and asserts those golden outputs satisfy the skill's non-negotiable
...
def run_should_use_fixture(fixture_dir: Path, meta: dict) -> list:
    response_text = (fixture_dir / "golden_output.md").read_text()
    failures = []
