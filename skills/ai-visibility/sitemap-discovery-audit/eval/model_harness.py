#!/usr/bin/env python3
"""Gated, credentialed model-harness layer for sitemap-discovery-audit.
...
def build_enabled_system_prompt() -> str:
    ...
    return (
        ...
        "to (out of scope, no site given, an implementation request rather than an "
        "audit, or a request for something this skill does not do), say so plainly "
        "instead of forcing the skill's report shape.\n\n"
        f"--- SKILL.md ---\n{skill_md}\n\n"
...
def score_response(meta: dict, response_text: str) -> list:
    if meta["category"] == "should_use":
        result = contract.check_report_contract(response_text, meta)
