#!/usr/bin/env python3
"""Gated, credentialed model-harness layer for ecommerce-technical-seo-audit.
...
def build_enabled_system_prompt() -> str:
    ...
    return (
        ...
        "to (not an ecommerce catalog spot-check request, wrong skill's job, a vague "
        "ask with no concrete page to check, a request to run a full-catalog crawl "
        "this skill explicitly does not do, or a request this skill should refuse per "
        "its guardrails), say so plainly instead of forcing the skill's output "
        "shape.\n\n"
        f"--- SKILL.md ---\n{skill_md}\n\n"
