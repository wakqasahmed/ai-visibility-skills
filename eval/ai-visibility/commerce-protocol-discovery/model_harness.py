#!/usr/bin/env python3
"""Gated, credentialed model-harness layer for commerce-protocol-discovery.
...
skill's own SKILL.md, references/checks.md, and the shared
references/guardrails.md text - no other context, chat history, tools, or
network access. The same contract.py validator used by run_eval.py scores
the model's response, so both layers agree on what "correct" means.
...
def build_enabled_system_prompt() -> str:
    ...
    guardrails_md = (SKILL_DIR / "references" / "guardrails.md").read_text()
    return (
        ...
        "to (not an ecommerce/marketplace commerce-protocol-discovery request, a "
        "non-ecommerce site, a request for scored readiness/remediation that belongs "
        "to a different, private audit capability, or a request this skill should "
        "refuse per its guardrails), say so plainly instead of forcing the skill's "
        "output shape.\n\n"
        f"--- SKILL.md ---\n{skill_md}\n\n"
        f"--- references/checks.md ---\n{checks_md}\n\n"
        f"--- references/guardrails.md ---\n{guardrails_md}"
    )
...
def score_response(meta: dict, response_text: str) -> list:
    if meta["category"] == "should_use":
        result = contract.check_discovery_contract(
            response_text,
