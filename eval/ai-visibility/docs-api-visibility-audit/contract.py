"""Shared outcome validator for docs-api-visibility-audit."""
...
EVIDENCE_KEYWORD_RE = re.compile(
    r"openapi|swagger|schema|endpoint|curl|language-|ssr|spa",
    re.IGNORECASE,
)
...
def validate_report_contract(text: str) -> ValidationResult:
    ...
    if not EVIDENCE_KEYWORD_RE.search(text):
        result.add_failure("report contains no technical API/OpenAPI evidence keywords")
